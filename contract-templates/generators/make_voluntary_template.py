#!/usr/bin/env python3
"""自願離職協議書（員工主動提出離職）範本 — 以黎陸梅版為基礎，單頁 A4。"""
...
def set_font(p, size=11, bold=False):
    for run in p.runs:
...
rf = OxmlElement('w:rFonts'); rf.set(qn('w:eastAsia'), '標楷體'); rPr.append(rf)

p = doc.add_paragraph()
p.add_run('                      益循生活股份有限公司          （以下簡稱甲方）')
set_font(p)
p = doc.add_paragraph()
p.add_run('立協議人              ＿＿＿＿＿＿＿＿＿          （以下簡稱乙方）')
set_font(p)

p = doc.add_paragraph()
p.paragraph_format.space_after = Pt(4)
p.add_run('乙方自民國（下同）＿＿年＿＿月＿＿日起於甲方任職，經乙方於＿＿年＿＿月＿＿日提出自願離職，爰雙方合意於＿＿年＿＿月＿＿日（以下稱「離職日」）終止雙方之僱傭關係，雙方權益關係約定如下：')
set_font(p)

clauses = [
    ('一、', '甲方應於確認乙方履行第四條之義務後，給付共計新台幣（下同）＿＿＿＿＿＿＿整作為乙方之離職金（包括但不限於任職期間之補償、賠償、勞務報酬、獎金、福利、加班費、特別休假未休工資、預告期間工資及其他在職期間和本協議約定所生之一切款項）。'),
    ('二、', '乙方保證於離職日後不得再就任職期間之勞動關係，對甲方進行任何法律上之請求或主張，包括但不限於向勞工或其他行政主管機關之檢舉或申訴、提起民事訴訟或刑事告訴、告發。'),
    ('三、', '乙方於甲方任職時所知悉所有資訊（包括但不限於客戶資訊、營運細節及報價）及本協議書全部內容，均應視為機密事項，乙方不得將前述資訊散佈或揭露予任何第三人（包括但不限於甲方員工）知悉，雙方均不得有傳送、散佈對他方、他方人員及他方關係企業不利之言論或行為。'),
    ('四、', '乙方應於離職日前將甲方指定之工作內容完整交予甲方指定之第三人。'),
...
p = doc.add_paragraph(); p.add_run('戶籍地址：＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿'); set_font(p)
